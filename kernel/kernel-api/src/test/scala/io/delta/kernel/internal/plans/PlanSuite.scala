...
class PlanSuite extends AnyFunSuite {
  ...
  test("Values and UnionAll form a topologically ordered plan") {
    val left = values(1, 2)
    val right = values(3)
    val plan = new Plan(Seq(
      node(left),
      node(right),
      node(UnionAll.UNION_ALL, 0, 1, 0)).asJava)

    assert(plan.getNodes.size() === 3)
    assert(plan.getNodes.get(2).getInputs.asScala === Seq(0, 1, 0))
    assert(plan.getOutputSchema(0) === schema)
    assert(plan.getOutputSchema === schema)
    assert(left.getRows.asScala.map(_.getLong(0)) === Seq(1L, 2L))
    assert(right.getRows.asScala.map(_.getLong(0)) === Seq(3L))
  }
  ...
  test("operators enforce input counts and exact schemas") {
