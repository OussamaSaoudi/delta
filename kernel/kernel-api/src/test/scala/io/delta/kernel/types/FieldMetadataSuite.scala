...
class FieldMetadataSuite extends AnyFunSuite {
  ...
  test("equal metadata has equal hashes for null, array, and nested values") {
    val nested1 = FieldMetadata.builder().putLong("version", 1L).build()
    val nested2 = FieldMetadata.builder().putLong("version", 1L).build()

    val meta1 = FieldMetadata.builder()
      .putNull("null")
      .putString(null, "null-key")
      .putLongArray("array", Array[java.lang.Long](1L, null, 3L))
      .putFieldMetadata("nested", nested1)
      .putFieldMetadataArray("nested-array", Array(nested1))
      .build()
    val meta2 = FieldMetadata.builder()
      .putFieldMetadataArray("nested-array", Array(nested2))
      .putFieldMetadata("nested", nested2)
      .putLongArray("array", Array[java.lang.Long](1L, null, 3L))
      .putString(null, "null-key")
      .putNull("null")
      .build()

    assertThat(FieldMetadata.empty().hashCode()).isEqualTo(0)
    assertThat(meta1).isEqualTo(meta2)
    assertThat(meta1.hashCode()).isEqualTo(meta2.hashCode())
  }

  test("hash follows builder reuse and array mutation") {
    val builder = FieldMetadata.builder().putString("first", "value")
    val metadata = builder.build()
    val initialHash = metadata.hashCode()

    builder.putString("second", "value")
    assertThat(metadata.hashCode()).isNotEqualTo(initialHash)
    assertThat(metadata).isEqualTo(builder.build())

    val values = Array[java.lang.Long](1L, 2L)
    val arrayMetadata = FieldMetadata.builder().putLongArray("values", values).build()
    val initialArrayHash = arrayMetadata.hashCode()
    values(1) = 3L
    assertThat(arrayMetadata.hashCode()).isNotEqualTo(initialArrayHash)
  }

  test("equals distinguishes missing keys from null values") {
    val left = FieldMetadata.builder().putNull("left").build()
    val right = FieldMetadata.builder().putNull("right").build()
