...
public final class Agg {
  ...
  StructField outputField(StructType inputSchema, String outputName) {
    if (key != null) {
      PlanSchemaUtils.resolveField(inputSchema, key, "Aggregate key");
    }
    StructField valueField = PlanSchemaUtils.resolveField(inputSchema, value, "Aggregate value");
    String name = outputName == null ? defaultOutputName() : outputName;
    return new StructField(
        name, PlanSchemaUtils.stripFieldMetadata(valueField.getDataType()), true);
  }
