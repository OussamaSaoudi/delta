...
final class AggregateOperator {
  ...
  static CloseableIterator<ColumnarBatch> open(Aggregate node, OperatorDispatcher execution) {
    ...
    try {
      ...
      state =
          bound.groupCount == 0
              ? StateTable.global(execution.engine(), bound.valueSlotCount)
              : StateTable.keyed(
                  execution.engine(), bound.keySchema, bound.valueSlotCount);
  ...
  private static final class AggregateIterator extends ManagedIterator<ColumnarBatch> {
    ...
    private void consume(ColumnarBatch batch) {
      ...
      for (int rowId = 0; rowId < batch.getSize(); rowId++) {
        Row row = operands == null ? null : operands.getRow(rowId);
        int entryRef = 0;
  ...
  private static final class BoundAgg {
    ...
    private BoundAgg(
      ...
      this.valueType = aggregate.valueType();
      this.keyType = aggregate.keyType().orElse(valueType);
      this.valueSchema = new StructType().add(new StructField("_value", valueType, true));
      this.keySchema = new StructType().add(new StructField("_key", keyType, true));
    ...
    private void updateMinMax(StateTable state, int entryRef, Row operands) {
      ...
      Row current = state.getValue(entryRef, outputSlot);
      if (current == null
          || improves(RowKernels.compare(valueType, operands, valueOrdinal, current, 0))) {
        state.setValue(
            entryRef, outputSlot, engine.retainValue(operands, valueOrdinal, valueSchema));
      }
    }
    ...
    private void updateNonNullBy(StateTable state, int entryRef, Row operands) {
      ...
      Row currentKey = state.getValue(entryRef, orderingSlot);
      if (currentKey == null
          || improves(RowKernels.compare(keyType, operands, keyOrdinal, currentKey, 0))) {
        state.setValue(
            entryRef, orderingSlot, engine.retainValue(operands, keyOrdinal, keySchema));
        state.setValue(
            entryRef, outputSlot, engine.retainValue(operands, valueOrdinal, valueSchema));
      }
    }
