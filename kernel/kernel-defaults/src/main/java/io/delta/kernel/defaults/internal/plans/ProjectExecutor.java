...
final class ProjectExecutor {
  ...
  static CloseableIterator<FilteredColumnarBatch> execute(
    ...
    return input.map(batch -> evaluateBatch(inputSchema, outputSchema, evaluator, batch));
  }
  ...
  private static FilteredColumnarBatch evaluateBatch(
    ...
    try {
      ...
      checkArgument(
          ...
          root.getSize(),
          data.getSize());
      return batch.withData(resultBatch(outputSchema, root));
  ...
  private static ColumnarBatch resultBatch(StructType schema, ColumnVector root) {
    ColumnVector projectedRoot = new DefaultViewVector(root, 0, root.getSize());
