...
##### Method `getEvaluator(StructType inputSchema, Expression expression, StructType outputSchema)`

This method returns an [`ExpressionEvaluator`](https://delta-io.github.io/delta/snapshot/kernel-api/java/io/delta/kernel/expressions/ExpressionEvaluator.html) bound to one struct-valued expression. Evaluating a `FilteredColumnarBatch` returns another `FilteredColumnarBatch` with the same row count, row order, and selection. Its top-level columns are the fields of `outputSchema`. The same evaluator can process multiple input batches with `inputSchema`; each output batch declares whether its data is owned or borrowed.

##### Method `getPredicateEvaluator(StructType inputSchema, Predicate predicate)`
This method returns a [`PredicateEvaluator`](https://delta-io.github.io/delta/snapshot/kernel-api/java/io/delta/kernel/expressions/PredicateEvaluator.html) bound to the predicate. It accepts and returns a `FilteredColumnarBatch`, preserving the input data and narrowing its selection. A row that is unselected in the input remains unselected in the result, so predicates can be chained without rewriting batch data.
...
* The data in each `FilteredColumnarBatch` passed to an evaluator is guaranteed to have the input schema provided when the evaluator was created. It is therefore safe to bind expression evaluation to column ordinals instead of names.
